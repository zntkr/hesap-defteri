...
class DiscountToolWidget(BaseToolWidget):
    ...
    def build_ui(self) -> None:
        ...
        for i, (text, key) in enumerate(discount_items):
            tk.Label(discount_res_frame, text=text, fg=self.ui.text_secondary, bg=self.ui.bg_secondary, font=self.ui.font_main).grid(row=i, column=0, sticky="w", pady=4)
            lbl = tk.Label(discount_res_frame, text="-", font=self.ui.font_bold, fg=self.ui.fg_color, bg=self.ui.bg_secondary, cursor="hand2")
            lbl.grid(row=i, column=1, sticky="w", padx=24)
