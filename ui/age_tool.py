...
class AgeToolWidget(BaseToolWidget):
    ...
    def build_ui(self) -> None:
        self._build_header(self, "Detaylı Yaş Analizi", "Örn: 15.05.1990 girerek doğduğunuz günü, tam yaşınızı ve sonraki doğum gününüze kalan süreyi bulun.")
        age_frame = tk.Frame(self, bg=self.ui.bg_color)
        age_frame.pack(fill="x", pady=5)
        
        self.age_year_entry = self._build_input_row(age_frame, 0, "Doğum Tarihi:", "GG.AA.YYYY")
        ...
        self.age_res_txt.bind('<Button-1>', lambda e: self.ui.main_view._copy_to_clipboard(self.age_res_txt.get("1.0", "end-1c").strip(), self.age_info_lbl, "Hesaplandı • Kopyalamak için sonuca tıklayın"))
            
        self.age_info_lbl = self._build_info_label(self, "Analiz raporunu görmek için doğum tarihinizi girin", pad_y=(15, 0))
    ...
    def calculate_age(self, event: Optional[tk.Event] = None) -> Optional[str]:
        ...
        if date_str == "GG.AA.YYYY": date_str = ""
        ...
        if "hata" in result:
            ...
            self.age_info_lbl.config(text=err_msg, fg=self.ui.error_color)
            self._set_result("-")
            return "break"
